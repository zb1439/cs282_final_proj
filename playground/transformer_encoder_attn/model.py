...
@MODELS.register()
class SelfAttentionXFormer(BaseModel):
    def _init_modules(self):
        ...
        self.xformer = nn.Transformer(self.small_dim, num_encoder_layers=3, num_decoder_layers=1, batch_first=True)
        self.query = nn.Parameter(torch.Tensor(1, self.small_dim))
        nn.init.xavier_normal_(self.query)
        self.attn = nn.TransformerEncoder(nn.TransformerEncoderLayer(self.small_dim, 8, batch_first=False), 3)
        self.predictor = nn.Linear(self.small_dim, 1)
    ...
    def predict(self, user_feat, item_feat):
        ...
        bs = sparse_feat.size(0)
        item_size = sparse_feat.size(1)
        sparse_feat = sparse_feat.view(-1, sparse_feat.size(2) // self.small_dim, self.small_dim)

        nn.TransformerDecoder(nn.TransformerDecoderLayer(self.small_dim, 8), 3)

        sparse_feat = self.xformer(
            sparse_feat, 
            self.query.data.unsqueeze(0).tile(sparse_feat.size(0), 1, 1)
        )
        sparse_feat = sparse_feat.squeeze(-2).view(bs, item_size, -1)
